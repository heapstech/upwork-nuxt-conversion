...
const demoMode = (function () {
  ...
  const navPositionToggle = document.querySelectorAll(
    'input[name="navPosition"]'
  )
  ...
  const config = {
    showPopover: localStorage.getItem('dashkitShowPopover')
  ...
  function togglePopover() {
    if (popover) {
      const showPopover =
        JSON.parse(config.showPopover) && config.sidebarSize === 'base'
  ...
  function toggleFormControls(
    ...
    $(form)
      ...
      .button('toggle')
    $(form)
      .find('[name="navColor"][value="' + navColor + '"]')
      .closest('.btn')
      .button('toggle')
    $(form)
      .find('[name="sidebarSize"][value="' + sidebarSize + '"]')
      .closest('.btn')
      .button('toggle')
  }
